...
class Classy {
	...
	protected function __construct() {
		...
		add_action('acf/init',array($this,'load_template_function'));
		//$this->load_template_function();

		add_action('acf/init',array($this,'init_config'));
		//$this->init_config();
	 ...
	 */
	public function load_template_function() {
		require_once( CLASSY_THEME_PATH . 'app/functions/aq_resizer.php' );
		require_once( CLASSY_THEME_PATH . 'app/functions/template-functions.php' );
		require_once( CLASSY_THEME_PATH . 'app/functions/core-functions.php' );
		require_once( CLASSY_THEME_PATH . 'app/functions/typography-functions.php' );
		if(file_exists(CLASSY_THEME_PATH . 'theme-files/views/floshortcodes/custom-functions.php')){
			include_once( CLASSY_THEME_PATH . 'theme-files/views/floshortcodes/custom-functions.php' );
		}
	...
	private function define_constants() {
		...
		define( 'CLASSY_CHILD_THEME_PATH', get_stylesheet_directory() . '/' );
		define( 'CLASSY_CHILD_THEME_DIR', get_stylesheet_directory_uri() . '/' );
	...
	public static function textdomain() {
		...
		$textdomain = Classy::get_config_var( 'flotheme' );
	...
	public static function render( $view = null, $data = null ) {
		...
		if(CLASSY_CHILD_THEME_PATH != CLASSY_THEME_PATH){ // if the current template is a child theme
			$file_path = Hierarchy::get_file_path( $type = 'view', $view );
			$file_path_child_theme = str_replace(CLASSY_THEME_PATH,CLASSY_CHILD_THEME_PATH,$file_path);
			if(file_exists($file_path_child_theme)){
				$views = CLASSY_CHILD_THEME_PATH . View::$folder;
			}else{
				$views = CLASSY_THEME_PATH . View::$folder;
			}
		}else{
