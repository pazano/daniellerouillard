...
class Hierarchy {
	...
	protected static function check_request() {
		...
		$supported_custom_post_types = array('gallery','video');
		$supported_custom_post_types = apply_filters('supported_post_types_hierarchy',$supported_custom_post_types);
		...
		elseif ( is_front_page() && !is_home() ) : 
			if ( self::is_classy_template() ){
				return 'classy-template';
			}else{
				return 'page'; // if a page is set as home page
			}
		...
		elseif ( in_array(get_post_type(), $supported_custom_post_types) /*get_post_type() == 'gallery'*/ ) : return 'single_custom_post';
	...
	public static function file_exists( $type = 'view', $file ) {
		...
		if(CLASSY_CHILD_THEME_PATH != CLASSY_THEME_PATH){ // if the current template is a child theme
			$file_path_child = str_replace(CLASSY_THEME_PATH, CLASSY_CHILD_THEME_PATH, $file_path);

			if(file_exists( $file_path_child )){
				return true;
			}
		}
	...
	private static function get_request_hierarchy_list( $type ) {
		...
		elseif ( 'single_custom_post' === $type ) :
