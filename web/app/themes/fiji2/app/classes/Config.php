...
class Config {
	...
	public static function get_vars() {
		...
		if ( null === self::$vars ) {
			...
			$config_file = CLASSY_THEME_PATH . 'theme-files/app/config.php';

			if ( file_exists( $config_file ) ) {

				require_once CLASSY_THEME_PATH . 'theme-files/app/config.php';
				...
				wp_die( sprintf(
					'There is no config file in %s theme-files/app//config.php',
	...
	private static function init_sidebars( $sidebars ) {
		...
		// bellow we will register the custom sidebars added by the users
		$custom_sidebars = get_field('flo-sidebars__custom_sidebars','options');

		if (is_array($custom_sidebars) && sizeof($custom_sidebars) ) {
			
			/* register dinamic sidebar */
			if (function_exists('register_sidebar')) {
					foreach ($custom_sidebars as $sidebar) {
						if (isset($sidebar['flo-sidebar']) && !empty($sidebar['flo-sidebar'])) {
							register_sidebar(array(
									'name' => $sidebar['flo-sidebar'],
									'id' => trim(strtolower(str_replace(' ', '-', $sidebar['flo-sidebar']))),
									'before_widget' => $before_widget,
									'after_widget' => $after_widget,
									'before_title' => $before_title,
									'after_title' => $after_title,
