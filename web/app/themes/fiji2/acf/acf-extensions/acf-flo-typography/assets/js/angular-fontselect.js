...
(function(angular, undefined) {
  ...
  var id = 1;
    ...
    /**
     *
     * Custom directive for adding a new font block dinamically
     *
     */
    fontselectModule.directive('jdAddBlock', function ($compile) {
        return {
            restrict: 'E',
            // scope: {
            //     text: '@'
            // },
            template: '<div class="font-style-add" ng-click="add()">Add Google Font</div>',
            controller: function ($scope, $element) {
                $scope.add = function () {
                    var option_key = Math.floor((Math.random() * 10000) + 1),  //random number between 1 and 10000

                        // gfont_block = '<div class="group_google_font google_font_'+option_key+'">'+
                        //                     '<span ng-click="deleteGoogleFont(\'google_font_'+option_key+'\')">Delete Google font</span>'+
                        //                     '<div class="fontselect">'+
                        //                         '<jd-fontselect stack="google_font_'+option_key+'"></jd-fontselect>'+
                        //                     '</div>'+
                        //                     '<section class="content mw" style="font-family: {{google_font_'+option_key+'}};">'+
                        //                         '<input type="hidden" value="{{google_font_'+option_key+'}}" name="'+field_name+'[g_fonts][google_font_'+option_key+']" />' +
                        //                         '<p>Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. </p>' +
                        //                     '</section>'+
                        //                 '</div>'; 
                        gfont_block = '<div class="acf-field group_google_font google_font_'+option_key+'">'+
                                        '<div class="acf-label">'+
                                          '<label >{{google_font}}</label>'+
                                        '</div>'+
                                        '<div class="acf-input">'+
                                          '<div class="font-style-block">'+
                                            '<div class="font-style-block__top-bar">'+
                                              '<div class="font-style-block__top-bar-item" ng-click="deleteGoogleFont(\'google_font_'+option_key+'\')">'+
                                            ...
                                            '<div class="fontselect">'+
                                              '<jd-fontselect stack="google_font_'+option_key+'"></jd-fontselect>'+
                                            '</div>'+
                                            '<div class="font-style-block__paragraph" style="font-family: {{google_font_'+option_key+'}};">'+
                                              '<input type="hidden" value="{{google_font_'+option_key+'}}" name="'+field_name+'[g_fonts][google_font_'+option_key+']" />'+
                    ...
                    var el = $compile(gfont_block)($scope);
                    jQuery($element).parent().parent().before(el); //append the new element to the main block
                };
            }
        };
    });
      ...
      controller: [
        ...
        $scope.$watch('current.font', function(newFont, oldFont) {
          ...
          if (oldFont !== newFont) {
            ...
            //console.log(oldFont,newFont);

            if(typeof newFont !== 'undefined'){
              // build the object that holds the new font info in the format
              // compatible with the fonts_list used in the Typography styles
              var newFontListObject = {
                'font_type': 'google_font',
                'font_family': newFont.stack,
                'font_info': {
                  '0': {
                    'url': '',
                    'name': newFont.name,
              ...
              //console.log($scope.$parent.fonts_list);



              if(typeof $scope.$parent.fonts_list !== 'undefined'){
                // remove the font from the dropdown as well
                if(typeof oldFont !== 'undefined'){
                  delete $scope.$parent.fonts_list[oldFont.stack];
                }
                // add the new font to the Font Styles dropdown
                $scope.$parent.fonts_list[newFont.stack] = newFontListObject;
              }
            }
  ...
  FontsService.prototype = {
    ...
    _initGoogleFonts: function(force) {
      ...
      //   return;
      // }
      if ( (!force && _googleFontsInitiated)) {
        return;
      }
      ...
      if (self.jdFontselectConfig.googleApiKey){
          self.$http({
            ...
            (function(){
                var response = JSON.parse(flo_google_fonts);
                var amount = response.items.length;
                var ready = amount - 1;
                var fonts = [];
