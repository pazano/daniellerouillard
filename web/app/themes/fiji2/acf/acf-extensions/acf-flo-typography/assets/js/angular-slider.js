        ...
        compile: function() {
            var preLink = function (scope, elm, attrs, ngModel) {
                ...
                elm.bind(updateOn, function(event, ui) {
                    ...
                    jQuery(ui.handle).find('.ui-slider-tip').text(ui.value);
                    scope.$apply();
                ...
                scope.$watch(attrs.ngModel, function() {
                    if (options.range === true) {
                        ...
                        jQuery(elm).find('.ui-slider-tip').each(function(i, tipElm) {
                            jQuery(tipElm).text(ngModel.$viewValue[i]);
                        });
                    } else {
                        jQuery(elm).find('.ui-slider-tip').text(ngModel.$viewValue);
                    }
                }, true);
            ...
            var postLink = function (scope, element, attrs, ngModel) {
                ...
                if (angular.isDefined(options['tick']) && angular.isDefined(options['step'])) {
                    ...
                    for (var i = total; i >= 0; i--) {
                        ...
                        jQuery("<div/>").addClass("ui-slider-tick").appendTo(element).css({left: left});
                    };
                }
                if(angular.isDefined(options['tip'])) {
                    $timeout(function(){
                        ...
                        if(handles && handles.length>1 && ngModel.$viewValue && angular.isArray(ngModel.$viewValue)){
                            jQuery(handles[0]).append('<div class="ui-slider-tip">'+ngModel.$viewValue[0]+'</div>');
                            jQuery(handles[1]).append('<div class="ui-slider-tip">'+ngModel.$viewValue[1]+'</div>');
                        }else{
